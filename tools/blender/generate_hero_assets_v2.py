"""
Generate the Rocket Workshop hero asset pack v2.

Run from the repository root:
    blender --background --python tools/blender/generate_hero_assets_v2.py

Optional:
    blender --background --python tools/blender/generate_hero_assets_v2.py -- --engine CYCLES
    blender --background --python tools/blender/generate_hero_assets_v2.py -- --engine EEVEE
    blender --background --python tools/blender/generate_hero_assets_v2.py -- --skip-renders

The script creates only versioned v2 outputs. It does not edit Godot scenes,
...
SOURCE_DIR = ROOT / "assets_3d" / "source" / "v2"
EXPORT_DIR = ROOT / "assets_3d" / "export" / "v2"
RENDER_DIR = ROOT / "assets_3d" / "renders" / "v2"
REPORT_DIR = ROOT / "docs" / "builds"
BLEND_PATH = SOURCE_DIR / "rocket_workshop_hero_v2.blend"
REPORT_PATH = REPORT_DIR / "hero_assets_v2_report.md"

MANAGED_PROP = "hero_assets_v2_managed"
ROLE_PROP = "hero_assets_v2_role"
ASSET_PROP = "hero_assets_v2_asset"
...
RENDER_OUTPUTS = {
    "hero": RENDER_DIR / "hero_workbench_v2_1920x1080.png",
    "asset_sheet": RENDER_DIR / "asset_sheet_v2_1920x1080.png",
    "pet_closeup": RENDER_DIR / "pet_bottle_closeup_v2_1920x1080.png",
    "material_closeups": RENDER_DIR / "material_closeups_v2_1920x1080.png",
    "wireframe": RENDER_DIR / "wireframe_debug_v2_1920x1080.png",
}
...
def create_materials() -> None:
    ...
    create_material("MAT_PET_Edge", (0.82, 1.0, 0.98, 0.62), 0.22, alpha=0.62, transmission=0.45)
    create_material("MAT_PET_Scuff", (0.96, 1.0, 0.96, 0.26), 0.70, alpha=0.26)
    create_material("MAT_Paper_OffWhite", (0.94, 0.88, 0.70, 1.0), 0.91, bump_strength=0.030, bump_scale=70.0)
    create_material("MAT_Paper_Edge", (0.74, 0.66, 0.47, 1.0), 0.92, bump_strength=0.024, bump_scale=74.0)
    create_material("MAT_Cardboard", (0.64, 0.43, 0.22, 1.0), 0.95, bump_strength=0.055, bump_scale=86.0)
    create_material("MAT_Cardboard_Edge", (0.36, 0.24, 0.12, 1.0), 0.98, bump_strength=0.045, bump_scale=96.0)
    create_material("MAT_Wood_Base", (0.54, 0.34, 0.18, 1.0), 0.78, bump_strength=0.035, bump_scale=42.0)
    create_material("MAT_Plastic_Dark", (0.13, 0.15, 0.15, 1.0), 0.66)
    create_material("MAT_Metal_Soft", (0.60, 0.61, 0.56, 1.0), 0.52, metallic=0.12)
    create_material(
        "MAT_Indicator",
        (0.08, 0.96, 0.58, 1.0),
        0.22,
    ...
    create_material("MAT_Wood_Workbench", (0.68, 0.46, 0.26, 1.0), 0.78, bump_strength=0.045, bump_scale=38.0)
    create_material("MAT_Wood_Workbench_Dark", (0.38, 0.23, 0.12, 1.0), 0.84)
    create_material("MAT_Tape_Clear", (0.98, 0.94, 0.72, 0.44), 0.15, alpha=0.44, transmission=0.30)
    create_material("MAT_Tape_Core", (0.58, 0.38, 0.20, 1.0), 0.92, bump_strength=0.034, bump_scale=72.0)
    create_material("MAT_Backdrop_Neutral", (0.34, 0.36, 0.34, 1.0), 0.78)
    create_material("MAT_Closeup_Backdrop", (0.62, 0.66, 0.64, 1.0), 0.82)
    create_material("MAT_Scratch_Dark", (0.30, 0.19, 0.10, 1.0), 0.90)
    create_material("MAT_Stain", (0.28, 0.18, 0.10, 0.32), 0.92, alpha=0.32)
...
def build_pet_bottle() -> None:
    ...
    for ring_index, (z, radius) in enumerate(profile):
        for seg in range(segments):
            ...
            lower = max(0.0, min(1.0, (-0.84 - z) / 0.38))
            foot_wave = 1.0 + lower * (0.105 * math.cos(angle * 5.0) + 0.026 * math.cos(angle * 10.0))
            hand_wobble = 1.0 + 0.012 * math.sin(angle * 3.0 + z * 3.7) + 0.006 * math.cos(angle * 7.0 - z)
            dent = -0.022 * math.exp(-((z - 0.22) ** 2) / 0.26) * max(0.0, math.cos(angle - 0.65))
            dent += -0.014 * math.exp(-((z + 0.36) ** 2) / 0.18) * max(0.0, math.cos(angle + 2.15))
    ...
    for index in range(5):
        ...
        create_ellipsoid(
            ...
            (math.cos(angle) * 0.247, math.sin(angle) * 0.247, -1.176),
            (0.070, 0.032, 0.024),
            mat_edge,
            collection,
            "canonical",
    ...
    for index, (z, angle, length) in enumerate(
        ...
        radius = 0.392 if z < 0.8 else 0.18
...
def build_paper_cone() -> None:
    ...
    for seg in range(segments):
        angle = math.tau * seg / segments
        wobble = 1.0 + 0.034 * math.sin(angle * 3.0 + 0.35) + 0.014 * math.cos(angle * 7.0)
        dent = -0.018 * max(0.0, math.cos(angle - 1.35))
        z_offset = 0.010 * math.sin(angle * 5.0)
        vertices.append((math.cos(angle) * (base_radius * wobble + dent), math.sin(angle) * base_radius, z_offset))
    ...
    for index, crease_angle in enumerate([math.radians(-22.0), math.radians(96.0), math.radians(158.0)], start=1):
        crease_width = math.radians(1.8)
        crease_vertices: list[tuple[float, float, float]] = []
        for z, radius in [(0.075, base_radius * 0.83), (height * 0.64, base_radius * 0.30)]:
            for angle in [crease_angle - crease_width, crease_angle + crease_width]:
                crease_vertices.append((math.cos(angle) * radius, math.sin(angle) * radius, z))
        create_mesh_object(
            f"Paper_Nose_Cone_Soft_Fold_{index}",
            crease_vertices,
            [(0, 1, 3, 2)],
            collection,
            mat_edge,
...
def build_cardboard_fin() -> None:
    ...
    for side in [-thickness * 0.5, thickness * 0.5]:
        for index, (x, z) in enumerate(points):
            edge_wobble = 0.010 * math.sin(index * 1.8 + x * 4.0)
            warp = 0.012 * math.sin(x * 5.5 + z * 2.0)
...
def build_workbench() -> None:
    ...
    for index, (x, y, length, angle) in enumerate(scratch_layout):
        create_beveled_box(
    ...
    for index, (x, y, sx, sy) in enumerate([(-1.8, 0.72, 0.30, 0.16), (1.24, -0.82, 0.24, 0.13), (0.05, 1.05, 0.18, 0.10)], start=1):
        ...
        stain.scale = (sx, sy, 1.0)
        bpy.context.view_layer.objects.active = stain
        stain.select_set(True)
        bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
        stain.select_set(False)
...
def create_props() -> None:
    ...
    for index, (x, y, rot, sx, sy) in enumerate(
        [(-2.12, 0.58, 13.0, 0.34, 0.20), (2.10, 0.70, -12.0, 0.36, 0.18)],
        start=1,
    ):
        create_beveled_box(
    ...
    create_beveled_box("Prop_Abstract_Small_Tool_Handle", (0.34, 0.075, 0.035), (-2.28, -0.20, 0.122), STATE.materials["MAT_Plastic_Dark"], collection, "environment", rotation=(0.0, 0.0, math.radians(-18.0)), bevel_width=0.014)
    create_beveled_box("Prop_Abstract_Small_Tool_Tip", (0.18, 0.038, 0.025), (-2.08, -0.27, 0.126), STATE.materials["MAT_Metal_Soft"], collection, "environment", rotation=(0.0, 0.0, math.radians(-18.0)), bevel_width=0.009)
...
def create_scene_instances() -> None:
    ...
    create_asset_instance("pet_bottle", "Hero_PET_Bottle", hero_col, "hero", (-0.36, -0.06, 0.49), (0.0, math.radians(88.0), math.radians(-15.0)))
    create_asset_instance("paper_nose_cone", "Hero_Paper_Nose_Cone", cone_col, "hero", (0.96, -0.86, 0.165), (0.0, math.radians(82.0), math.radians(22.0)), (0.92, 0.92, 0.92))
    create_asset_instance("cardboard_fin", "Hero_Cardboard_Fin_A", fin_col, "hero", (-0.42, -0.58, 0.116), (math.radians(1.5), 0.0, math.radians(20.0)), (0.96, 0.96, 0.96))
    create_asset_instance("cardboard_fin", "Hero_Cardboard_Fin_B", fin_col, "hero", (0.52, 0.78, 0.116), (math.radians(-2.5), 0.0, math.radians(-18.0)), (0.90, 0.90, 0.90))
    create_asset_instance("cardboard_fin", "Hero_Cardboard_Fin_C", fin_col, "hero", (-1.00, 0.82, 0.116), (math.radians(2.0), 0.0, math.radians(44.0)), (0.86, 0.86, 0.86))
    create_asset_instance("tape_roll", "Hero_Tape_Roll", tape_col, "hero", (-1.88, -0.70, 0.185), (0.0, 0.0, math.radians(-8.0)), (0.92, 0.92, 0.92))
    create_asset_instance("launch_stand", "Hero_Launch_Stand", stand_col, "hero", (1.76, 0.55, 0.115), (0.0, 0.0, math.radians(-5.0)), (0.92, 0.92, 0.92))

    create_beveled_box("Hero_Fin_A_Discreet_Stain", (0.16, 0.004, 0.045), (-0.18, -0.47, 0.143), STATE.materials["MAT_Cardboard_Edge"], fin_col, "hero", rotation=(0.0, 0.0, math.radians(22.0)), bevel_width=0.001, bevel_segments=1)
    create_beveled_box("Hero_Fin_B_Discreet_Stain", (0.12, 0.004, 0.035), (0.74, 0.81, 0.143), STATE.materials["MAT_Cardboard_Edge"], fin_col, "hero", rotation=(0.0, 0.0, math.radians(-15.0)), bevel_width=0.001, bevel_segments=1)

    create_beveled_box(
        "Asset_Sheet_Neutral_Base",
        (6.4, 3.2, 0.035),
        (0.0, 0.0, -0.035),
        STATE.materials["MAT_Closeup_Backdrop"],
        sheet_col,
        "sheet",
        bevel_width=0.0,
    )
    create_asset_instance("pet_bottle", "Sheet_PET_Bottle", sheet_col, "sheet", (-1.22, 0.82, 0.30), (0.0, math.radians(89.0), math.radians(-8.0)), (0.54, 0.54, 0.54))
    ...
    create_beveled_box(
        "Material_Closeups_Neutral_Base",
        (5.8, 3.0, 0.035),
        (0.0, 0.0, -0.04),
        STATE.materials["MAT_Closeup_Backdrop"],
        sheet_col,
    ...
    create_asset_instance("pet_bottle", "Material_PET_Closeup", sheet_col, "material", (-1.52, 0.78, 0.28), (0.0, math.radians(89.0), math.radians(-10.0)), (0.50, 0.50, 0.50))
    create_asset_instance("cardboard_fin", "Material_Cardboard_Closeup", sheet_col, "material", (-0.42, 0.78, 0.08), (0.0, 0.0, math.radians(12.0)), (1.10, 1.10, 1.10))
    create_asset_instance("paper_nose_cone", "Material_Paper_Closeup", sheet_col, "material", (0.90, 0.76, 0.11), (0.0, math.radians(83.0), math.radians(8.0)), (0.82, 0.82, 0.82))
    create_asset_instance("tape_roll", "Material_Tape_Closeup", sheet_col, "material", (-1.18, -0.78, 0.17), (0.0, 0.0, math.radians(-8.0)), (1.05, 1.05, 1.05))
    create_beveled_box("Material_Wood_Closeup_Block", (0.84, 0.46, 0.08), (0.20, -0.82, 0.04), STATE.materials["MAT_Wood_Workbench"], sheet_col, "material", bevel_width=0.025, bevel_segments=3)
    create_beveled_box("Material_Wood_Closeup_Scratch", (0.42, 0.014, 0.006), (0.22, -0.78, 0.086), STATE.materials["MAT_Scratch_Dark"], sheet_col, "material", rotation=(0.0, 0.0, math.radians(-8.0)), bevel_width=0.002, bevel_segments=1)
    create_beveled_box("Material_Base_Plastic_Block", (0.52, 0.32, 0.08), (1.36, -0.86, 0.04), STATE.materials["MAT_Plastic_Dark"], sheet_col, "material", bevel_width=0.018, bevel_segments=2)
    create_beveled_box("Material_Base_Metal_Bar", (0.52, 0.055, 0.055), (1.36, -0.86, 0.13), STATE.materials["MAT_Metal_Soft"], sheet_col, "material", bevel_width=0.018, bevel_segments=2)
    create_cylinder("Material_Base_Indicator_Dot", 0.055, 0.025, (1.66, -0.86, 0.105), STATE.materials["MAT_Indicator"], sheet_col, "material", vertices=32)

    create_beveled_box(
        "PET_Closeup_Neutral_Base",
        (3.4, 1.9, 0.035),
        (0.0, 0.0, -0.04),
        STATE.materials["MAT_Closeup_Backdrop"],
        sheet_col,
    ...
    create_asset_instance("pet_bottle", "PET_Closeup_Bottle", sheet_col, "petdetail", (0.0, 0.02, 0.38), (0.0, math.radians(89.0), math.radians(-9.0)), (0.72, 0.72, 0.72))
...
def hide_canonical_objects() -> None:
    for obj in bpy.data.objects:
        if obj.get(ROLE_PROP) == "canonical":
            ...
            obj.hide_render = True
        elif obj.get(ROLE_PROP) == "material":
            obj.hide_render = True
        elif obj.get(ROLE_PROP) == "petdetail":
            obj.hide_render = True
        elif obj.get(ROLE_PROP) in {"hero", "environment"}:
            obj.hide_render = False


def set_role_render_visibility(*, hero: bool, sheet: bool, environment: bool, material: bool = False, petdetail: bool = False, canonical: bool = False) -> None:
    for obj in bpy.data.objects:
        ...
        if role == "hero":
            ...
            obj.hide_render = not material
        elif role == "petdetail":
            obj.hide_render = not petdetail
...
def create_background() -> None:
    ...
    create_beveled_box("Workshop_Back_Wall", (6.4, 0.08, 2.8), (0.0, 1.86, 1.23), material, collection, "environment", bevel_width=0.0)
    create_beveled_box("Workshop_Left_Soft_Panel", (0.08, 3.2, 2.1), (-3.04, 0.18, 1.02), material, collection, "environment", bevel_width=0.0)
...
def create_lighting() -> None:
    ...
    lights = [
        ("Warm_Key_Area", "AREA", (0.0, -2.9, 3.15), (-58.0, 0.0, 8.0), 680.0, 4.6, (1.0, 0.84, 0.64)),
        ("Cool_Fill_Area", "AREA", (-2.8, 0.15, 2.05), (-45.0, 0.0, -48.0), 245.0, 4.1, (0.70, 0.79, 1.0)),
        ("Right_Workbench_Fill", "AREA", (2.8, -0.4, 1.75), (-48.0, 0.0, 52.0), 165.0, 3.2, (1.0, 0.92, 0.80)),
        ("PET_Rim_Area", "AREA", (2.7, 1.5, 1.55), (-38.0, 0.0, 132.0), 210.0, 1.35, (0.78, 0.92, 1.0)),
    ]
...
def configure_render(engine_option: str) -> None:
    ...
    scene.world.color = (0.19, 0.20, 0.19)
...
def create_cameras_and_scene() -> None:
    create_background()
    create_lighting()
    create_camera("CAM_Hero", (3.25, -4.18, 2.08), (0.02, -0.06, 0.39), lens=50.0, dof=True)
    create_camera("CAM_AssetSheet", (0.0, -5.25, 3.05), (0.0, 0.0, 0.20), lens=55.0, ortho_scale=5.25)
    create_camera("CAM_PET_Closeup", (0.55, -2.45, 1.02), (0.0, 0.02, 0.38), lens=74.0, dof=True)
    create_camera("CAM_MaterialCloseups", (0.0, -5.05, 2.85), (0.0, 0.0, 0.18), lens=55.0, ortho_scale=4.75)
...
def render_outputs(skip_renders: bool) -> None:
    ...
    set_role_render_visibility(hero=True, sheet=False, environment=True, material=False, petdetail=False, canonical=False)
    render_still(RENDER_OUTPUTS["hero"], "CAM_Hero")

    set_role_render_visibility(hero=False, sheet=True, environment=False, material=False, petdetail=False, canonical=False)
    render_still(RENDER_OUTPUTS["asset_sheet"], "CAM_AssetSheet")

    set_role_render_visibility(hero=False, sheet=False, environment=False, material=False, petdetail=True, canonical=False)
    render_still(RENDER_OUTPUTS["pet_closeup"], "CAM_PET_Closeup")

    set_role_render_visibility(hero=False, sheet=False, environment=False, material=True, petdetail=False, canonical=False)
    render_still(RENDER_OUTPUTS["material_closeups"], "CAM_MaterialCloseups")
...
def validate_outputs(skip_renders: bool) -> dict[str, object]:
    ...
    pet_transparent_loose_rings = [
        obj.name
        for obj in canonical_objects_for_asset("pet_bottle")
        if obj.type == "MESH"
        and any(token in obj.name.lower() for token in ["ring", "ridge", "torus"])
    ]
    transforms_not_applied = [
        obj.name
        for obj in bpy.data.objects
        if obj.get(ROLE_PROP) == "canonical"
        and obj.type == "MESH"
        and (
            abs(obj.scale.x - 1.0) > 0.001
            or abs(obj.scale.y - 1.0) > 0.001
            or abs(obj.scale.z - 1.0) > 0.001
        )
    ]
    hero_root_positions = {
        obj.name: obj.location.copy()
        for obj in bpy.data.objects
        if obj.get(ROLE_PROP) == "hero" and obj.type == "EMPTY"
    }
    hero_interpenetration_warnings: list[str] = []
    distance_checks = [
        ("Hero_PET_Bottle", "Hero_Paper_Nose_Cone", 0.52),
        ("Hero_PET_Bottle", "Hero_Launch_Stand", 0.62),
        ("Hero_Tape_Roll", "Hero_Cardboard_Fin_A", 0.44),
    ]
    for first, second, min_distance in distance_checks:
        if first in hero_root_positions and second in hero_root_positions:
            delta = hero_root_positions[first] - hero_root_positions[second]
            planar_distance = math.hypot(delta.x, delta.y)
            if planar_distance < min_distance:
                hero_interpenetration_warnings.append(f"{first}/{second}: {planar_distance:.2f} < {min_distance:.2f}")
    godot_output_paths = [
        str(path)
        for path in [SOURCE_DIR, EXPORT_DIR, RENDER_DIR, REPORT_PATH]
        if "app\\godot" in str(path).lower().replace("/", "\\")
    ]
    ...
    validation = {
        ...
        "helpers_visible_in_render": helpers_visible,
        "pet_transparent_loose_rings": pet_transparent_loose_rings,
        "hero_interpenetration_warnings": hero_interpenetration_warnings,
        "transforms_not_applied": transforms_not_applied,
        "duplicate_object_names": duplicate_object_names,
        "orphaned_managed_objects": orphaned_managed_objects,
        "godot_output_paths": godot_output_paths,
    }
...
def print_summary() -> None:
    print("\nHero assets v2 generation complete.")
...
def main() -> None:
    ...
    set_role_render_visibility(hero=True, sheet=False, environment=True, material=False, canonical=False)
    save_blend()
